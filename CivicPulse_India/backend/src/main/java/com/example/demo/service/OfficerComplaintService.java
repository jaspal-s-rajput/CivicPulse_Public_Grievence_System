...
//                .build();
        ...
        OfficerComplaintResponse response = new OfficerComplaintResponse();

        response.setId(c.getId());
        response.setTitle(c.getTitle());
        response.setCategory(c.getCategory() != null ? c.getCategory().name() : null);
        response.setPriority(c.getPriority() != null ? c.getPriority().name() : null);
        response.setStatus(c.getStatus() != null ? c.getStatus().name() : null);
        response.setDescription(c.getDescription());
        response.setLocation(locationText);
        response.setLatitude(c.getLatitude());
        response.setLongitude(c.getLongitude());
        response.setSubmissionDate(c.getSubmissionDate());
        response.setImageUrl(
                c.getImageUrl() != null ? c.getImageUrl() : c.getOfficerEvidenceUrl()
        );
        response.setComplaintStage(
                c.getComplaintStage() != null ? c.getComplaintStage().name() : null
        );

        response.setAssignedOfficerName(
                assignedOfficer != null ? assignedOfficer.getName() : null
        );
        response.setAssignedOfficerStatus(
                assignedOfficer != null ? assignedOfficer.getStatus().name() : null
        );
        response.setAssignedOfficerDepartment(
                assignedOfficer != null ? assignedOfficer.getDepartment() : null
        );
        response.setAssignedOfficerActiveComplaints(
                assignedOfficer != null ? assignedOfficer.getActiveComplaints() : 0L
        );

        response.setOfficerRemark(c.getOfficerRemark());
        response.setAssignedDate(c.getAssignedDate());
        response.setExpectedCompletionDate(c.getExpectedCompletionDate());
