...
const AllComplaints = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [complaintsRes, officersRes] = await Promise.all([
        axios.get("http://localhost:8082/api/admin/complaints", axiosConfig),
        axios.get(
          "http://localhost:8082/api/admin/complaints/officers/workload",
          axiosConfig
        ),
      ]);
  ...
  const handleAssign = async (complaintId, officerId) => {
    try {
      await axios.post(
        `http://localhost:8082/api/admin/complaints/${complaintId}/assign-officer`,
        { officerId },
        axiosConfig
      );
      fetchData();
  ...
  const handleStatusChange = async (complaintId, newStatus) => {
    try {
      await axios.post(
        `http://localhost:8082/api/admin/complaints/${complaintId}/update-status`,
        { status: newStatus },
        axiosConfig
      );
      setComplaints((prev) =>
  ...
  const handlePriorityChange = async (complaintId, newPriority) => {
    try {
      await axios.post(
        `http://localhost:8082/api/admin/complaints/${complaintId}/update-priority`,
                  ...
                  <TableRow key={c.id}>
                    ...
                    <TableCell>
                      {c.officerEvidenceUrl ? (
                        <img
                          src={`http://localhost:8082${c.officerEvidenceUrl}`}
                          alt="Evidence"
                          style={{
                            width: 80,
                            borderRadius: 4,
                          ...
                          onClick={() =>
                            handleOpenPreview(`http://localhost:8082${c.officerEvidenceUrl}`)
                          }
                        />
                      ) : (
                        "-"
